...
impl Validity {
  ...
  pub fn arity(self, r: Range<usize>) -> Self {
    if !self.is_valid() {
      return self;
    }
  ...
  /// The first argument may be a point (Number) and/or Range.
  /// The second argument may be a point (Number) and/or Range.
  pub fn point_or_range(self, 
    ...
    match (first_type, second_type) {
      (FeelType::Number, FeelType::Number) => {
        if !first_allow_point {
          ExecutionLog::log(&format!(
            "Called {:?} with a Number for first argument, expected a Range", 
            self.name()
          ));
          self.and(false)
        }
          ...
          ExecutionLog::log(&format!(
            "Called {:?} with a Number for second argument, expected a Range", 
            self.name()
          ));
          self.and(false)
        }
      ...
      (FeelType::Number, FeelType::Range) => {
        if !first_allow_point {
          ExecutionLog::log(&format!(
            "Called {:?} with a Number for first argument, expected a Range", 
            self.name()
          ...
          ExecutionLog::log(&format!(
            "Called {:?} with a Range for second argument, expected a Number", 
            self.name()
          ));
          self.and(false)
        }
      ...
      (FeelType::Range, FeelType::Number) => {
        if !first_allow_range {
          ExecutionLog::log(&format!(
            "Called {:?} with a Range for first argument, expected a Number", 
            self.name()
          ));
          self.and(false)
        }
      ...
      _ => {
        ExecutionLog::log(&format!(
          "Called {:?} with an argument that was neither a Number nor a Range", 
          self.name()
        ));
        self.and(false)
      }
